...
=== "Python"
    In Python, `BlockId` is typically represented as a string string in the format `blk_...`, though a `BlockId` class exists for internal type safety.

    ```python
    # IDs are returned as strings from most API methods
    block_id: str = "blk_a1b2c3d4e5f6a1b2c3d4e5f6"
    ```

=== "JavaScript"
    In JavaScript, Block IDs are always represented as strings.

    ```javascript
    const blockId = "blk_a1b2c3d4e5f6a1b2c3d4e5f6";
    ```
...
=== "Python"
    ID generation happens automatically when adding blocks to a document.

    ```python
    # ID generated based on content and role
    id = doc.add_block(parent, "Hello, world!", role="intro")
    ```
...
=== "JavaScript"
    ...
    ```javascript
    // ID generated based on content and role
    const id = doc.addBlock(parent, "Hello, world!", "intro");
    ```
...
=== "Python"
    ```python
    root = doc.root_id
    print(root) # blk_ff00000000000000000000
    ```

=== "JavaScript"
    ```javascript
    const root = doc.rootId;
    console.log(root); // blk_ff00000000000000000000
    ```
