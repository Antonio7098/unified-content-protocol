 ...
 * ```
 */
export function writeSection(doc, sectionId, markdown, baseHeadingLevel) {
 ...
 */
export function clearSectionWithUndo(doc, sectionId) {
    if (!doc.blocks.has(sectionId)) {
 ...
 */
export function restoreDeletedSection(doc, content) {
 ...
 */
function clearSectionChildren(doc, sectionId) {
    const removed = [];
 ...
 */
function integrateBlocks(doc, parentId, sourceDoc, baseHeadingLevel) {
    const added = [];
...
function captureSectionContent(doc, sectionId) {
    const deleted = {
        parentId: sectionId,
        blocks: {},
        structure: {},
    ...
    return {
        ...
        metadata: block.metadata
            ? {
                ...
                modifiedAt: block.metadata.modifiedAt instanceof Date
...
function removeSubtree(doc, blockId) {
    const removed = [];
    const block = doc.blocks.get(blockId);
    if (!block) {
        return removed;
...
function findParentId(doc, childId) {
 ...
 */
function integrateSubtree(doc, parentId, sourceDoc, sourceBlockId, baseHeadingLevel, depth) {
    const added = [];
 ...
 * ```
 */
export function findSectionByPath(doc, path) {
 ...
 * ```
 */
export function getAllSections(doc) {
    const sections = [];
