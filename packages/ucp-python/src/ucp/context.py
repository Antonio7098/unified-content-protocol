...
class ContextWindow:
    ...
    def has_capacity(self) -> bool:
        return (
            self.block_count < self.constraints.max_blocks and
...
class ContextManager:
    ...
    def initialize_focus(
        ...
        while True:
            ...
            if parent is None or parent == doc.root or depth >= 3:
                break
            self._add_block_internal(doc, parent, InclusionReason.STRUCTURAL_CONTEXT, 0.8 - depth * 0.1)
    ...
    def expand_context(
        ...
        if direction == ExpandDirection.DOWN:
            ...
            result.blocks_added = (
                self._expand_downward(doc, focus_id, depth) +
                self._expand_upward(doc, focus_id, depth)
            )
    ...
    def get_statistics(self) -> ContextStatistics:
        ...
        average_relevance = (
            total_relevance / len(self.window.blocks)
            if self.window.blocks else 0.0
        )
        ...
        while queue and self.window.has_capacity():
            ...
            for child in doc.children(node_id):
                if not self.window.contains(child):
                    ...
                    self._add_block_internal(doc, child, InclusionReason.STRUCTURAL_CONTEXT, relevance)
                    added.append(child)
            ...
            if block and block.metadata:
                ...
                for edge in edges:
                    ...
                    if target and not self.window.contains(target) and target not in visited:
                        ...
                        self._add_block_internal(doc, target, InclusionReason.SEMANTIC_RELEVANCE, relevance)
                        added.append(target)
    ...
    def _prune_if_needed(self) -> List[str]:
        ...
        while (
            self.window.block_count > self.window.constraints.max_blocks or
    ...
    def _find_block_to_remove(self) -> Optional[str]:
        ...
        if self.pruning_policy == PruningPolicy.RELEVANCE_FIRST:
            candidates = [
                (bid, cb) for bid, cb in self.window.blocks.items()
                if bid != focus
            ]
            if candidates:
