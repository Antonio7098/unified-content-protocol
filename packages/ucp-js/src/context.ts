...
export class ContextWindow {
  readonly id: string
  readonly blocks: Map<BlockId, ContextBlock> = new Map()
  ...
  hasCapacity(): boolean {
    return (
      this.blockCount < this.constraints.maxBlocks &&
...
export class ContextManager {
  ...
  constructor(
    ...
    expansionPolicy: ExpansionPolicy = 'balanced',
    pruningPolicy: PruningPolicy = 'relevance_first'
  ) {
  ...
  initializeFocus(doc: Document, focusId: BlockId, taskDescription: string): ContextUpdateResult {
    ...
    while (depth < 3) {
      ...
      if (!parent || parent === doc.root) break
  ...
  addBlock(
    doc: Document,
    blockId: BlockId,
    reason: InclusionReason = 'external_decision'
  ): ContextUpdateResult {
    const result: ContextUpdateResult = {
      blocksAdded: [],
      blocksRemoved: [],
   ...
   */
  expandContext(doc: Document, direction: ExpandDirection, depth: number = 2): ContextUpdateResult {
    const result: ContextUpdateResult = {
      blocksAdded: [],
      blocksRemoved: [],
      blocksCompressed: [],
   ...
   */
  compress(doc: Document, method: CompressionMethod = 'truncate'): ContextUpdateResult {
    const result: ContextUpdateResult = {
      blocksAdded: [],
      blocksRemoved: [],
      blocksCompressed: [],
  ...
  getStatistics(): ContextStatistics {
    ...
    for (const cb of this.window.blocks.values()) {
      ...
      if (cb.compressed) compressedCount++
    }
  ...
  private expandDownward(doc: Document, start: BlockId, maxDepth: number): BlockId[] {
    const added: BlockId[] = []
    const queue: Array<{ id: BlockId; depth: number }> = [{ id: start, depth: 0 }]
  ...
  private expandUpward(doc: Document, start: BlockId, maxDepth: number): BlockId[] {
    const added: BlockId[] = []
    let current = start
    let depth = 0
  ...
  private expandSemantic(doc: Document, start: BlockId, maxDepth: number): BlockId[] {
    const added: BlockId[] = []
  ...
  private pruneIfNeeded(): BlockId[] {
    const removed: BlockId[] = []

    while (
      this.window.blockCount > this.window.constraints.maxBlocks ||
  ...
  private findBlockToRemove(): BlockId | undefined {
