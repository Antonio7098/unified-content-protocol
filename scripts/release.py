...
def ensure_clean_worktree() -> None:
    ...
    if status:
        raise SystemExit("Working tree has uncommitted changes. Commit or stash before releasing.")
...
def ensure_on_branch(expected: str | None) -> None:
    ...
    if branch != expected:
        raise SystemExit(f"Releases must run from {expected}, current branch is {branch}.")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run release checks and optionally tag the repo.")
    parser.add_argument("--branch", help="Require running from this branch.", default="main")
    parser.add_argument("--tag", action="store_true", help="Create and push a git tag after checks.")
    parser.add_argument("--push", action="store_true", help="Push the created tag to origin.")
