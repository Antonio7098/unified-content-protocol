...
impl UcpClient {
    ...
    pub fn execute_ucl(&self, doc: &mut Document, ucl: &str) -> Result<Vec<OperationResult>> {
        let commands = parse_commands(ucl)
    ...
    /// Add a code block  
    pub fn add_code(&self, doc: &mut Document, parent: &BlockId, lang: &str, code: &str) -> Result<BlockId> {
        ...
        for cmd in commands {
            match cmd {
                ucl_parser::Command::Edit(e) => {
                    let block_id: BlockId = e.block_id.parse()
                ...
                ucl_parser::Command::Append(a) => {
                    let parent_id: BlockId = a.parent_id.parse()
                    ...
                    ops.push(Operation::Append {
                        parent_id,
                        content,
                        label: a.properties.get("label").and_then(|v| match v { ucl_parser::Value::String(s) => Some(s.clone()), _ => None }),
                        tags: Vec::new(),
                        semantic_role: a.properties.get("role").and_then(|v| match v { ucl_parser::Value::String(s) => Some(s.clone()), _ => None }),
                        index: a.index,
                    });
                }
                ucl_parser::Command::Delete(d) => {
                    if let Some(id) = d.block_id {
                        let block_id: BlockId = id.parse().map_err(|_| Error::InvalidBlockId(id.clone()))?;
                        ops.push(Operation::Delete { block_id, cascade: d.cascade, preserve_children: d.preserve_children });
                    }
                }
                ucl_parser::Command::Move(m) => {
                    let block_id: BlockId = m.block_id.parse().map_err(|_| Error::InvalidBlockId(m.block_id.clone()))?;
                    match m.target {
                        ucl_parser::MoveTarget::ToParent { parent_id, index } => {
                            let new_parent: BlockId = parent_id.parse().map_err(|_| Error::InvalidBlockId(parent_id.clone()))?;
                            ops.push(Operation::Move { block_id, new_parent, index });
                        }
                ...
                ucl_parser::Command::Prune(p) => {
                    let condition = match p.target {
                        ucl_parser::PruneTarget::Unreachable => Some(ucm_engine::PruneCondition::Unreachable),
                        _ => None,
                    };
                    ops.push(Operation::Prune { condition });
                }
                ucl_parser::Command::Link(l) => {
                    let source: BlockId = l.source_id.parse().map_err(|_| Error::InvalidBlockId(l.source_id.clone()))?;
                    let target: BlockId = l.target_id.parse().map_err(|_| Error::InvalidBlockId(l.target_id.clone()))?;
                    let edge_type = ucm_core::EdgeType::from_str(&l.edge_type).unwrap_or(ucm_core::EdgeType::References);
                    ops.push(Operation::Link { source, edge_type, target, metadata: None });
                }
                ucl_parser::Command::Snapshot(s) => {
                    match s {
...
mod tests {
    ...
    fn test_add_text() {
        ...
        let id = client.add_text(&mut doc, &root, "Hello, world!", Some("intro")).unwrap();
