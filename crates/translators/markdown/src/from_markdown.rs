...
impl MarkdownParser {
    ...
    pub fn parse(&self, markdown: &str) -> Result<Document> {
        ...
        while i < lines.len() {
            ...
            if let Some(heading) = self.parse_heading(line) {
                ...
                let block_id = doc.add_block(block, &parent)
            ...
            let current_parent = heading_stack.iter().rev()
                .find_map(|h| h.clone())
            ...
            if line.starts_with("```") {
                ...
                doc.add_block(block, &current_parent).map_err(|e| TranslatorError::InvalidStructure(e.to_string()))?;
                i += consumed;
                continue;
            }
    ...
    fn is_list_item(&self, line: &str) -> bool {
        ...
        trimmed.starts_with("+ ") || Self::is_ordered_item(trimmed)
    }
    ...
    fn parse_table(&self, lines: &[&str]) -> Result<(Content, usize)> {
        ...
        while i < lines.len() && lines[i].contains('|') {
            // Skip separator row
            if lines[i].chars().all(|c| c == '|' || c == '-' || c == ':' || c == ' ') {
                i += 1;
                continue;
            }
...
mod tests {
    ...
    fn test_heading_semantic_roles() {
        ...
        let headings: Vec<_> = doc.blocks.values()
...
"#;
        ...
        let root_children = doc.structure.get(&doc.root).expect("Root should have children");
        assert_eq!(root_children.len(), 1, "Root should have exactly one H1 child");
        ...
        assert_eq!(h1_children.len(), 2, "H1 should have 2 H2 children (Section 1 and Section 2)");
        
        // First H2 (Section 1) should have content and H3 as children
        let h2_section1_id = &h1_children[0];
        let h2_section1_children = doc.structure.get(h2_section1_id).expect("Section 1 should have children");
        assert!(h2_section1_children.len() >= 2, "Section 1 should have content and subsection");
    }
...
"#;
        ...
        // Find the H1 block
        let h1_block = doc.blocks.values()
            .find(|b| {
                b.metadata.semantic_role.as_ref()
                    .map(|r| r.category == RoleCategory::Heading1)
                    .unwrap_or(false)
        ...
        let h1_children = doc.structure.get(&h1_block.id).expect("H1 should have children");
        assert!(h1_children.len() >= 2, "H1 should have paragraph and H2 as children");
    }
...
"#;
        ...
        // Find the list block
        let list_block = doc.blocks.values()
            .find(|b| {
                b.metadata.semantic_role.as_ref()
        ...
        assert!(list_block.is_some(), "Should have list block with List semantic role");
    }
...
"#;
        ...
        let para_block = doc.blocks.values()
            .find(|b| {
                b.metadata.semantic_role.as_ref()
        ...
        assert!(para_block.is_some(), "Should have paragraph block with Paragraph semantic role");
    }
...
"#;
        ...
        let quote_block = doc.blocks.values()
            .find(|b| {
                b.metadata.semantic_role.as_ref()
        ...
        assert!(quote_block.is_some(), "Should have quote block with Quote semantic role");
    }
}
